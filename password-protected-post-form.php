...
<form class="bullet" action="<?php print esc_url( get_option( 'siteurl' ) . '/wp-pass.php' ); ?>" method="post">
	...
	<label class="bullet-label" for="<?php print esc_attr( 'password-form-' . get_the_id() ); ?>"><?php _e( 'Enter Password', 'nighthawk' ); ?></label>

	<input class="bullet-term" name="post_password" id="<?php print esc_attr( 'password-form-' . get_the_id() ); ?>" type="password" size="20" />
